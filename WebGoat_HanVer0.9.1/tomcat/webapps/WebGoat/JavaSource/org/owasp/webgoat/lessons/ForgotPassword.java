...
public class ForgotPassword extends LessonAdapter
{
	...
	private final static Hashtable USERS = new Hashtable();
	...
	protected Element doStage1(WebSession s)
	{
		...
		ec.addElement( new BR().addElement( new H1().addElement( "Webgoat 비밀번호 복구 " )));
		...
		TR tr = new TR();
		tr.addElement( new TH().addElement("당신의 사용자명을 입력하십시오.  계정이 없다면 OWASP admin으로 로그인 후 보십시오.See the OWASP admin if you do not have an account.")
		...
		Element b = ECSFactory.makeButton( "실행" );
	...
	protected Element doStage2(WebSession s)
	{
		...
		ec.addElement( new H1().addElement( "Webgoat 비밀번호 복구 " ));
		Table t = new Table().setCellSpacing( 0 ).setCellPadding( 2 ).setBorder( 0 ).setWidth("90%").setAlign("center");
		...
		TR tr = new TR();
		tr.addElement( new TH().addElement("비밀 질의: 당신이 가장 좋아하는 색은 무엇입니까?")
				.setColSpan(2).setAlign("left"));
		t.addElement( tr );
		...
		TR row1 = new TR();
		row1.addElement( new TD( new B( new StringElement( "*답: " ) ) ));
	...
	protected Element doStage3(WebSession s)
	{
		...
		TR tr = new TR();
		tr.addElement( new TH().addElement("보안을 위해, 당신의 비밀번호를 즉시 변경하십시오.")
				.setColSpan(2).setAlign("left"));
		t.addElement( tr );
		...
		tr = new TR();
		tr.addElement( new TD().addElement( new StringElement("사용자명: " + USERNAME_RESPONSE)));
		t.addElement ( tr );
		
		tr = new TR();
		tr.addElement(new TD().addElement( new StringElement("색깔: " + COLOR_RESPONSE)));
		t.addElement ( tr );
		
		tr = new TR();
		tr.addElement(new TD().addElement( new StringElement("비밀번호: " + USERS.get(USERNAME_RESPONSE).toString())));
		t.addElement(tr);
	...
	protected Element createContent( WebSession s )
	{
		...
		if(STAGE == 2)
		{
			...
			{
				s.setMessage("" + USERNAME_RESPONSE + "의 응답과 올바르지 않습니다. 다시 시도하십시오!");
		...
		{				
			...
			else
			{
				if(username.length() > 0)
					s.setMessage("타당한 사용자명이 아닙니다. 다시 시도하십시오.");
	...
	public List getHints()
	{
		...
		hints.add( "어떤 방법으로 잠금이 풀리지 않는다면, 당신의 방법은 brute force 이다!" );
		hints.add( "Try using usernames you might사용자명을 쓰려고 시도한다 당신은  encounter마주친다 throughout WebGoat." );
		hints.add( "매우 많은 색깔들 중에서 하나를 알아맞출수 있습니까?" );
		hints.add( "관리자의 계정은 \"admin\"" );
	...
	{
		String instructions = "누군가 그외의 비밀번호를 화면에 출력해준다. 당신의 사용자명은 'webgoat' 이고 당신이 가장좋아하는 색은 'red'";
		return instructions;
	}
