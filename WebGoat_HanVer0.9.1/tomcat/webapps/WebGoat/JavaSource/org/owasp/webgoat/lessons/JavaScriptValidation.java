...
public class JavaScriptValidation extends LessonAdapter
{
	...
	protected Element createContent( WebSession s )
	{
		...
		String script = "<SCRIPT>" + lineSep +
				...
				"if (!regex1.test(form.field1.value)) {err+=1; msg+='\\n  bad field1';}" + lineSep +
				"if (!regex2.test(form.field2.value)) {err+=1; msg+='\\n  bad field2';}" + lineSep +
				"if (!regex3.test(form.field3.value)) {err+=1; msg+='\\n  bad field3';}" + lineSep +
				"if (!regex4.test(form.field4.value)) {err+=1; msg+='\\n  bad field4';}" + lineSep +
				"if (!regex5.test(form.field5.value)) {err+=1; msg+='\\n  bad field5';}" + lineSep +
				"if (!regex6.test(form.field6.value)) {err+=1; msg+='\\n  bad field6';}" + lineSep +
				"if (!regex7.test(form.field7.value)) {err+=1; msg+='\\n  bad field7';}" + lineSep +
				"if ( err > 0 ) alert(msg);" + lineSep +
				"else form.submit();" + lineSep +
				"} " + lineSep + 
				"</SCRIPT>" + lineSep;
		...
		try
		{
			...
			ec.addElement( new Div().addElement( new StringElement( "Field1: 정확히 소문자 3개를 입력하시오.exactly three lowercase characters (" + regex1 + ")" ) ) );

			ec.addElement( new Div().addElement( input1 ) );

			ec.addElement( new P() );

			ec.addElement( new Div().addElement( new StringElement( "Field2: 정확히 숫자 3개를 입력하시오.exactly three digits (" + regex2 + ")" ) ) );

			ec.addElement( new Div().addElement( input2 ) );

			ec.addElement( new P() );

			ec.addElement( new Div().addElement( new StringElement( "Field3: 문자letters, 숫자numbers, 그리고 and space only (" + regex3 + ")" ) ) );

			ec.addElement( new Div().addElement( input3 ) );

			ec.addElement( new P() );

			ec.addElement( new Div().addElement( new StringElement( "Field4: 숫자들의 나열enumeration of numbers (" + regex4 + ")" ) ) );

			ec.addElement( new Div().addElement( input4 ) );

			ec.addElement( new P() );

			ec.addElement( new Div().addElement( new StringElement( "Field5: 단순한 zip code 입력 simple zip code (" + regex5 + ")" ) ) );
			...
			if ( !pattern1.matcher( param1 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation: Field1 작업 성공.You succeeded for Field1.";
			...
			if ( !pattern2.matcher( param2 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation: Field2 작업 성공.You succeeded for Field2.";
			...
			if ( !pattern3.matcher( param3 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation: Field3 작업 성공.You succeeded for Field3.";
			...
			if ( !pattern4.matcher( param4 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation:  Field4 작업 성공.You succeeded for Field4.";
			...
			if ( !pattern5.matcher( param5 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation:  Field5 작업 성공.You succeeded for Field5.";
			...
			if ( !pattern6.matcher( param6 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation:  Field6 작업 성공.You succeeded for Field6.";
			...
			if ( !pattern7.matcher( param7 ).matches() )
			{
				...
				msg += "<BR>Server side validation violation:  Field7 작업 성공.You succeeded for Field7.";
	...
	protected List getHints()
	{
		...
		hints.add( "당신의 브라우저에 사건이 발생했음을 확인합니다.The validation is happening in your browser." );
		hints.add( "그들이 당신의 브라우저를 떠날때 프록시를 통해서 값을 수정한다.Try modifying the values with a proxy after they leave your browser" );
		hints.add( "또다른 방법은 당신이 페이지를 보기전에 자바스크립트를 지웁니다.Another way is to delete the JavaScript before you view the page." );
	...
	public String getInstructions(WebSession s)
	{
		...
		String instructions = "이 웹사이트유형은 클라이언트도 서버도 확인합니다.This website performs both client and server side validation.  " +
