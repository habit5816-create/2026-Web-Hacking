...
public class CommandInjection extends LessonAdapter
{
	...
	protected Element createContent( WebSession s )
	{
		...
		try
		{
			...
			if ( s.isDefuseOSCommands() && ( helpFile.indexOf('&') != -1 || helpFile.indexOf(';') != -1) ) 
			{
				...
				if ( helpFile.substring(index + 1).trim().toLowerCase().equals("netstat -a") ||
				...
				else
				{
					s.setMessage("It appears that you are on the right track.  " +
								"명령어는 운영체제 장애에 원이이 될수도 있습니다.  " +
								"다음과 같은 명령을 사용할 수 있습니다.: netstat -a, dir, ls, ifconfig, and ipconfig");
				}
			}
			...
			if ( s.isDefuseOSCommands() &&  helpFile.indexOf('&') == -1 && helpFile.indexOf(';') == -1 )
			{
				...
				{	
					...
					else 
					{
						s.setMessage("It appears that you are on the right track.  " +
									"명령어는 운영체제 장애에 원이이 될수도 있습니다.  " +
							    	"여기는 커맨드 인젝선을 배우는곳이며 , 접근제어를 배우는곳이 아닙니다..");
					}
				}
				else
				{
			...
			if ( !illegalCommand ) {
				...
				ec.addElement( new StringElement( splitMessage(fileData.replaceAll(System.getProperty("line.separator"),"<br>")
					      .replaceAll("(?s)<!DOCTYPE.*/head>","")
					      .replaceAll("<br><br>","<br>")
					      .replaceAll("<br>\\s<br>","<br>"),_80,true)));
	...
	protected List getHints()
	{
		...
		hints.add( "시스템 명령을 사용하는 응용 프로그램은 파일의 내용을 반환." );
		hints.add( "&기호는 윈도우 2000 명령과 쉘명령을 분리하는게 사용합니다. 일반적으로 유닉스에서의 구분 기호는 ; 입니다." );
		hints.add( "프록시를 사용하여 & netstat -a & ipconfig를 삽입합니다." );
	...
	{
		String instructions = "이문서가 적용하려는 학습 계획을 선택합니다..  " +
			"운영체제에서 command injection을 시도해 보십시요.";
