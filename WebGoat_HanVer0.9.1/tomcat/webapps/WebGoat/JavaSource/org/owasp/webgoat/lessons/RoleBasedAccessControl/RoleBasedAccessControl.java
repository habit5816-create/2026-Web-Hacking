...
public class RoleBasedAccessControl extends LessonAdapter
{
	...
	protected List getHints()
	{
		...
		hints.add( "많은 사이트들은 리소스에 접속하는 것을 역할에 의해서 제한하려고 한다. Many sites attempt to restrict access to resources by role." );
		hints.add( "개발자들은 자주 이 계획을 구현 하는 실수를 범한다. Developers frequently make mistakes implementing this scheme." );
		hints.add( "사용자의 조합, 역할, 그리고 리소스에 시도한다. Attempt combinations of users, roles, and resources." );
		
		// Stage 1
		hints.add( "어떻게 어플리케이션이 사용자가 선택한 삭제 함수를 알 수 있는가? How does the application know that the user selected the delete function?" );
		
		// Stage 2
		
		// Stage 3
		hints.add( "어떻게 어플리케이션이 사용자가 선택한 어떤 특정 직원을 보는 것을 알 수 있는가? How does the application know that the user selected any particular employee to view?" );

		// Stage 4
		hints.add( "주목할 것은 그 직원의 컨텐츠 목록은 누가 로그인 되어있는가에 따라서 다르다는 것이다. Note that the contents of the staff listing change depending on who is logged in." );
	...
	public String getInstructions(WebSession s)
	{
		...
		if (!getLessonTracker(s).getCompleted())
		{
			...
			{
			case 1:
				instructions = "Stage " + getStage(s) + ": 기능 액세스 제어를 파괴한다. Breaking functional access control.<br>" +
					"당신은 정규 직원으로 로그인할 수 있어야 하고 다른 사용자의 직원을 삭제할 수 있어야 한다. You should be able to login as a regular employee and delete another user's employee " +
					"윤곽을 그려보면, 비록 인사부서만 쓸수 있는 기능이라고 할 지라도. profile, even though that is supposed to be an HR-only function.";
				break;
			case 2:
				instructions = "Stage " + getStage(s) + ": 비즈니스 계층에서 액세스 제어를 구현한다. Implementing access control in the Business Layer<br>"
					+ "액세스 제어는 이미 프리젠테이션 계층에 구현이 되어 있으나 우리가 보기에는 충분하지 않다.Access control has already been implemented in the Presentation Layer, but as we have just " +
					"seen, this is not enough.  Implement access control in the Businesss Layer to verify " + 
					"비지니스 계층에서 삭제 기능 사용을 실제로 실행하기전에 권한 검증을 하기 위해서 액세스 제어를 구현한다.authorization to use the Delete function before actually executing it.";
				break;
			case 3:
				instructions = "Stage " + getStage(s) + ": 데이터 액세스 제어를 파괴한다.Breaking data access control.<br>" +
				...
				break;
			case 4:
				instructions = "Stage " + getStage(s) + ": 데이터 계층에서 액세스 제어를 구현한다.Implementing access control in the Data Layer.<br>" +
					"인증되지 않은(그리고 잠재적인 위협 경력이 있는) 직원 개인 정보에 액세스하는 것을 방지하기 위하여 데이터 계층 액세스 제어를 구현한다.Implement Data Layer access control to prevent unauthorized (and potentially career threatening) " +
