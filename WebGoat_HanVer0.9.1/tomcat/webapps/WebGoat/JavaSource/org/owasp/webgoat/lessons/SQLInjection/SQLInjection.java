...
public class SQLInjection extends LessonAdapter
{
	...
	protected List getHints()
	{
		...
		hints.add( "어플리케이션은 당신의 입력과 이미 만들어진 SQL문 끝에 넣은 것을 가질 것이다.The application is taking your input and inserting it at the end of a pre-formed SQL command." );
		hints.add( "이것은 WebGoat에 의해 만들어지고 나온 쿼리를 위한 코드이다. This is the code for the query being built and issued by WebGoat:<br><br> " +
					"\"SELECT * FROM employee WHERE userid = \" + userId + \" and password = \" + password" );
		hints.add( "SQL문 혼합은 AND와 OR과 같은 키워드를 같이 넣어서 다중으로 합칠수 있어야한다.Compound SQL statements can be made by joining multiple tests with keywords like AND and OR.  " +
					"SQL문을 추가하려는 노력이 문제를 푸는데 도움이 된다. Try appending a SQL statement that always resolves to true");
		
		// Stage 1
		hints.add( "당신의 공격에 알맞게 필드 길이 제한을 없애기 위해서 WebScarab을 사용해야 될 수도 있다.You may need to use WebScarab to remove a field length limit to fit your attack." );
		hints.add( "패스워드에 입력한다.Try entering a password of [ smith' OR '1' = '1 ]." );

		// Stage 2
		hints.add( "WebGoat의 데이터베이스 쿼리의 다수는 이미 파라미터로 나타내어졌다. 프로젝트를 위해 준비된 구문을 검색하라. Many of WebGoat's database queries are already parameterized.  Search the project for PreparedStatement." );
		
		// Stage 3
		hints.add( "패스워드에 입력한다.[ 101 OR 1=1 ORDER BY 'salary' ]." );
	...
	public String getInstructions(WebSession s)
	{
		...
		if (!getLessonTracker(s).getCompleted())
		{
			...
			{
			case 1:
				instructions = "Stage " + getStage(s) + ": 우회 인증을 하기위해 문자열 SQL 인젝션을 사용한다. Use String SQL Injection to bypass authentication. "
					+ "여기서 목표는 Admin 그룹에 있는 사용자 The goal here is to login as the user "
					+ PRIZE_EMPLOYEE_NAME
					+ "로 로그인하는 것이다., who is in the Admin group.  "
					+ "비밀번호가 없어도 되지만 양식은 SQL 인젠션이 가능해야 한다. You do not have the password, but the form is SQL injectable.";
				break;
			case 2:
				instructions = "Stage " + getStage(s) + ": 파라미터로 나타내어진 쿼리를 사용한다. Use a parameterized query.<br>" +
					"동적 SQL쿼리는 사용하기 위해서 로그인 함수가 필요하지 않다. A dynamic SQL query is not necessary for the login function to work.  Change login " +
					"쿼리 매게변수안의 악의적인 SQL에게서 보호하기 위해 로그인 변경을 할때 나타내어진 쿼리를 사용한다. to use a parameterized query to protect against malicious SQL in the query parameters.";
				break;
			case 3:
				instructions = "Stage " + getStage(s) + ": 우회 인증을 하기위해 완전한 SQL 인젝션을 사용한다. Use Integer SQL Injection to bypass access control.<br>" +
					"여기에서의 목표는 저번 수업에 있는 자료 액세스 제어 체크와 같이 또, CEO의 직원 프로필을 보는 것이다. The goal here is to view the CEO's employee profile, again, even with data access " +
					"control checks in place from a previous lesson.  " +
					"전과 같이,비밀번호가 없어도 되지만 양식은 SQL 인젠션이 가능해야 한다. As before, you do not have the password, but the form is SQL injectable.";
				break;
			case 4:
				instructions = "Stage " + getStage(s) + ":파라미터로 나타내어진 쿼리를 다시 사용한다.  Use a parameterized query again.<br>" +
					"숫자 쿼리 매개변수에 있는 악의적인 SQL로 부터 보호하기 위해 ViewProfile 함수 변경에 나타내어진 쿼리를 사용한다. Change the ViewProfile function to use a parameterized query to protect against " +
