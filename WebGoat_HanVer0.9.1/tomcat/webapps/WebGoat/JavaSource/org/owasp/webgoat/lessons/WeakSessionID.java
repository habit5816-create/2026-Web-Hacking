...
public class WeakSessionID extends LessonAdapter {
     ...
     */
    protected final static String SESSIONID = "취약 ID";
    
    /**
     *  Description of the Field
     */
    protected final static String PASSWORD = "비밀 번호";
    
    /**
     *  Description of the Field
     */
    protected final static String USERNAME = "사용자이름";
    ...
    protected List getHints() {
        ...
        hints.add( "당신이 올바른 쿠키를 보낸다면 서버의 인증을 우회한다." );
        hints.add( "쿠키 값을 예상할수 있는가? 다른 누군가가 취득한 쿠키의 결함들을 볼수 있는가?" );
        hints.add( "좀더 시도해라, 둔한 친구야!" );  
    ...
    public String getInstructions(WebSession s) {
        String instructions = "다른 사람에게 할당된 세션에 접근을 시도하세요.";
    ...
    protected Element makeLogin( WebSession s ) {
        ...
        ec.addElement( new H1().addElement( " 서 명 " ));
        ...
        if (username != null || password != null) {
            s.setMessage("부적절한 사용자이름 또는 암호.");
        }
        
        TR tr = new TR();
        tr.addElement( new TH().addElement("당신의 계정에 서명하세요.")
        .setColSpan(2).setAlign("left"));
        t.addElement( tr );
        
        tr = new TR();
        tr.addElement( new TD().addElement("*요청된 필드들").setWidth("20%"));
        t.addElement( tr );
        ...
        TR row1 = new TR();
        TR row2 = new TR();
        row1.addElement( new TD( new B( new StringElement( "*사용자이름: " ) ) ));
        row2.addElement( new TD( new B(new StringElement( "*비밀 번호: " ) ) ));
